...
按「输出要求」与「审稿报告模板」输出中英文两份 Markdown 文件；如用户需要 Word 版本，转写为 .docx 并交付。
...
先写学术英语版本，再逐段翻译成中文。两份文件的首段、编号、问题严重程度、建议、引用和最终决定必须一一对应；任何一份都不能出现另一份没有的实质内容。

每份报告采用以下形式：

1. 首先是一段总体评价，概括研究问题、优点、创新、总体质量和主要局限。
2. 随后使用数字编号列出修改意见。每条尽量包含：定位、具体问题、为何重要、建议如何修改。
3. 最后单列 `Recommendation` / `处理建议`，在 Accept、Minor Revision、Major Revision、Reject 中选择，并用一句话说明理由。

整体话术应简洁、专业、克制。区分以下三种表述强度：
...
两份语言版本均按下列结构组织，内容一一对应。如投稿系统或用户需要，可将内部整理的「稿件信息摘要」表附于报告开头或末尾；默认报告仍以总体评价开头。

```markdown
# Peer Review Report / 审稿意见报告

## 一、总体评价（Summary）
（研究问题、主要优势、创新点、总体质量、主要局限；以当前形式是否达到目标期刊要求）

## 二、主要问题（Major Concerns）
1. **【问题标题】**（定位：第 X 章 / 表 X / 图 X）
   - 问题：……
   - 为何重要：……
   - 建议：……

## 三、次要问题（Minor Concerns）
1. **【问题标题】**（定位：……）
   - 问题与建议：……

## 四、Recommendation / 处理建议
...
- 意见已按 Major / Minor 分级，每条包含「问题—为何重要—建议—定位」。
